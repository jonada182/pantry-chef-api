...
def seedData(dbClient):
    try:
        ...
        current_app.logger.info("Seeding the MongoDB")
        ...
        if items_collection_name in db.list_collection_names():
            ...
            if items:
                ...
                current_app.logger.info("Data seeded to MongoDB")
            else:
                print("No items to insert")

            print("Items saved to MongoDB")
    except Exception as e:
        print(f'An error occurred when seeding data: {e}')
        current_app.logger.error(f'An error occurred when seeding data: {e}')
        return False
    finally:
        return True
